...
use crate::state::{Game, GameMove, GameResult, State, GAME, HOOKS, STATE};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::AddHook { address } => {
            Ok(HOOKS.execute_add_hook(&ADMIN, deps, info, api.addr_validate(&address)?)?)
        }
        ExecuteMsg::RemoveHook { address } => {
            Ok(HOOKS.execute_remove_hook(&ADMIN, deps, info, api.addr_validate(&address)?)?)
        }
...
mod tests {
    ...
    pub fn try_to_change_admin() {
        ...
        let _init_msg = InstantiateMsg {
            owner: info.sender.clone(),
            admin: Some("creator".to_string()),
        };
        let env = mock_env();

        instantiate(deps.as_mut(), mock_env(), info.clone(), _init_msg).unwrap();
